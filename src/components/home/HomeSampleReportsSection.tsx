    ...
    <section id="sample" className="min-h-screen flex flex-col bg-[#131313]">
      <div className="flex-1 grid lg:grid-cols-[2fr_3fr] content-wrap page-gutter w-full">
        ...
        <div className="flex flex-col justify-center py-20 lg:pr-16" style={{ borderRight: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <p className="text-base leading-[1.8] text-white/50 max-w-[34ch] mb-10">
          ...
          <Link href="/reports-and-outputs" className="text-sm font-semibold no-underline border-b border-[#2d6cff] pb-0.5 w-fit text-[#2d6cff] transition-opacity hover:opacity-70">
            View sample reports →
          </Link>
        </div>
        ...
        <div className="flex flex-col justify-center py-20 lg:pl-16 gap-0">
          {reportCards.map((card, i) => (
            <div key={i} className="group flex gap-8 py-8 border-b border-white/6 first:border-t first:border-white/6 px-2 -mx-2 hover:bg-white/2 transition-colors duration-300 cursor-default">
              <span className="text-base font-bold shrink-0 w-8 pt-0.5 text-white/20 group-hover:text-[#2d6cff] transition-colors duration-300">
                {card.num}
              </span>
              <div>
                <p className="text-xs font-bold tracking-[0.2em] uppercase text-[#2d6cff] opacity-60 group-hover:opacity-100 transition-opacity duration-300 mb-2">
                  {card.type}
                </p>
                <h3 className="text-base font-bold uppercase tracking-[0.02em] leading-[1.3] mb-2 text-white" style={{ fontFamily: "var(--font-space-grotesk)" }}>
                  {card.title}
                </h3>
                <p className="text-sm italic text-white/30 mb-3">{card.audience}</p>
                <p className="text-base leading-[1.7] text-white/50">{card.desc}</p>
              </div>
            </div>
          ))}
        </div>
