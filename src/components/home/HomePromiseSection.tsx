          ...
          {promises.map((card, i) => {
            ...
            return (
              <div key={i} className="group flex gap-8 py-8 px-4 -mx-4 rounded transition-colors duration-300 hover:bg-[rgba(45,108,255,0.06)] cursor-default" style={{ borderBottom: i < promises.length - 1 ? "1px solid rgba(187,220,251,0.08)" : "none" }}>
                <span className="text-lg font-bold shrink-0 w-8 pt-0.5 transition-colors duration-300 group-hover:text-[#2d6cff]" style={{ color: "rgba(234,239,243,0.25)" }}>
                  {num}
                </span>
                <div>
                  <h3 className="text-base font-bold uppercase tracking-[0.04em] mb-1 transition-colors duration-300 group-hover:text-[#6b93ff]" style={{ color: "#fff" }}>
                    {card.title}
                  </h3>
                  {label && <p className="text-sm font-medium italic mb-3 text-[#2d6cff] ">{label}</p>}
                  <p className="text-base leading-[1.7]" style={{ color: "rgba(234,239,243,0.55)" }}>
                    {card.body}
                  </p>
                </div>
              </div>
