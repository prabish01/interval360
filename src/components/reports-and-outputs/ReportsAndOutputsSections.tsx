  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0e0e0e]">
      ...
      <div className="content-wrap page-gutter w-full pt-28 pb-12">
        <p className="text-xs font-semibold tracking-[0.28em] uppercase text-[#2d6cff] mb-6">Reports &amp; Outputs</p>
            ...
            <div className="flex flex-col">
              {anatomySections.map((item, i) => (
                <div key={i} className="flex items-start gap-5 py-5 border-b border-white/6 first:border-t first:border-white/6">
                  <span className="text-sm font-bold w-8 shrink-0 pt-0.5 text-[#2d6cff]">{item.num}</span>
                  <div>
                    <h4 className="text-base font-bold uppercase mb-1 text-white">{item.title}</h4>
                    <p className="text-base leading-[1.65] text-white/50">{item.body}</p>
                  </div>
                </div>
              ))}
            </div>
